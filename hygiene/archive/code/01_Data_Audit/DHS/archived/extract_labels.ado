...
capture program drop extract_labels
program define extract_labels
syntax, survey(string) dataloc(string) keyloc(string) keyversion(string) prevtype(string)


** *************************************** NEW SECTION *********************************************
// FOR DEBUGGING ONLY!
//local survey dhs
//local dataloc 		"J:/WORK/01_covariates/02_inputs/water_sanitation/hygiene/data/prevalence/DHS"
...
//do "`code_folder'/Scripts/categorize_special.ado"


** open dataset with variable list and store variable data in a mata matrix
use "`dataloc'/varlist_`survey'", clear

mata: vars=st_sdata(.,("countryname", "filedir", "filename", "w_srcedrnk", "t_type"))
local maxobs = _N
...
forvalues filenum = 1(1)`maxobs' {
	// create locals with file-specific information
		** country and filename
		mata: st_local("countryname", vars[`filenum',1])
	...
	if regexm("`filename'", "IPUMS") {
		if $changed == 1 {
			local varlist_new `w_srcedr' toilet_type
	...
	forvalues varnum = 1(1)`varlistlength' {
		...
		** check to make sure that the variable has entries
		...
		if r(N) == 0 {	// no entries
			...
			local emptyvars `emptyvars' `countryname'_`var`varnum''
		}
		else {	// has entries
...
aorder
order countryname iso3 startyear endyear filename svy, first

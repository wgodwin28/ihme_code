...
# source("/home/j/WORK/05_risk/01_database/02_data/air_pm/04_paf/04_models/code/dev/01_calculate_paf.r", echo=T)
...
if (Sys.info()["sysname"] == "Linux") {
  ...
  root <- "J:"
  arg <- c("CHN_491", 2013, "power2", "gbd2010", "3", "adv_regress_draws2014_12_15.csv", 24, 1000, 15, TRUE, TRUE, FALSE)
}
...
rr.dir <- paste0(root, "/WORK/05_risk/01_database/02_data/air_pm/02_rr/04_models/output")

if (rr.version == "stan") {
  
  rr.functions <- paste0(root, "/WORK/05_risk/01_database/02_data/air_pm/02_rr/04_models/code/functional_forms_updated.r") # GBD 2013 version of RR (which uses Rstan) has a modified evaluation function which divides all parameters by 1e10 to avoid numbers that are too large for STAN to store in memory
  
} else {
  
  rr.functions <- paste0(root, "/WORK/05_risk/01_database/02_data/air_pm/02_rr/04_models/code/functional_forms.r") 
  
}

exp.grid.dir <- paste0(root, "/WORK/05_risk/01_database/02_data/air_pm/01_exp/02_nonlit/03_temp/gridded")
exp.reg.dir <- paste0(root, "/WORK/05_risk/01_database/02_data/air_pm/01_exp/04_models/results")

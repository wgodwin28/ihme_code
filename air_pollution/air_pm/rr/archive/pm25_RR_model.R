...
# working directory
data <- 'C:/Users/jfrostad/Desktop/RISK_LOCAL/air_pm/STAN/data/mean_updated'
graphs <- 'C:/Users/jfrostad/Desktop/RISK_LOCAL/air_pm/STAN/graphs/presentation'
diagnostic.output <- 'C:/Users/jfrostad/Desktop/RISK_LOCAL/air_pm/STAN/output/power2_non_informative_priors'
...
for (cause in causes) {
  ...
  for (age in age.list) {
    ...
    p <- p + geom_line(aes(y=mn), alpha=0.5, color='navy')
    p <- p + geom_point(aes(y=observed, size=Size, color=Type))
    p <- p + coord_trans(xtrans="log")
    p <- p + ggtitle(paste0(cause, '_', age)) + xlab('exposure') + ylab('RR')
    print(p)
    dev.off()
#     # plot a double logged version
#     rr_data_trunc <- rr_data[rr_data$exposure <= 300,]
#     pdf(file.path(graphs, paste0(cause,'_',age,'_rr_trunc.pdf')))
#     p <- ggplot(rr_data_trunc, aes(x=exposure))
#     p <- p + geom_ribbon(aes(ymin=lower, ymax=upper), alpha=0.5, color='steelblue', fill='steelblue')
#     p <- p + geom_line(aes(y=mn), alpha=0.5, color='navy')
#     p <- p + geom_point(aes(y=observed, size=Size), color='black')
#     p <- p + ggtitle(paste0(cause, '_', age)) + xlab('exposure') + ylab('RR')
#     print(p)
#     dev.off()      

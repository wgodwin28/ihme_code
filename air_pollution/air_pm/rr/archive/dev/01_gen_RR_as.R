...
# source("/home/j/WORK/05_risk/01_database/02_data/air_pm/02_rr/04_models/code/01_gen_RR_as.R", echo=T)
#********************************************************************************************************************************
...
if (Sys.info()["sysname"] == "Linux") {
  ...
  root <- "J:"
  arg <- c("GBR_433", "2000", "3", "stan", "power2", 100, 1)
}
...
prepped.environment <-  paste0(root, "/WORK/05_risk/01_database/02_data/smoking_direct/02_rr/04_models/data/prepped/clean.Rdata")
#objects imported:
#AS.global.exp - calculated PM exposure for smokers in a given country year, see calculation steps in master file 00_master_as.R
...
AS.country.exp <- AS.global.exp[iso3==this.country & year==this.year]
...
RR.mort <- mclapply(1:nrow(age.cause.sex), 
                    FUN=calculateRRs, 
                    exposure.object = AS.country.exp, 
                    metric.type = "yll", 
                    sex.specific = F,
                    function.cores = 1,

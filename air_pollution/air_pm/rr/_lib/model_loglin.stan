...
model {
  ...
  // make predictions for log(RR) using beta/gamma
  for (n in 1:N) {
  pred_log_rr[n] <- beta*log(exposure[n]/cf_exposure[n]) + gamma*(exposure[n]-cf_exposure[n]);
  }
...
generated quantities {
  ...
  for (t in 1:T) {
  predicted_RR[t] <- exp(beta*log(test_exposure[t]/tmrel) + gamma*(test_exposure[t]-tmrel));
  }
}
...
model {
  ...
  rho ~ gamma(1.0, 0.01);
  tmrel ~ gamma(1.0, 0.01);
  delta ~ gamma(1.0, 0.01);
  
  // make predictions for log(RR) using alpha/beta/gamma
  for (n in 1:N) {
  pred_log_rr[n] <- log(exp(beta * pow(((exposure[n] - tmrel)+fabs(exposure[n] - tmrel))/2, rho)) / exp(beta * pow(((cf_exposure[n] - tmrel)+fabs(cf_exposure[n] - tmrel))/2, rho)));
  }
  
  // setup the data likelihood
  // our data is distributed with sd of sd(log(RR)) and mean being our predicted log(RR)
  for (n in 1:N) {
  log_rr[n] ~ normal(pred_log_rr[n], sqrt(log_rr_sd[n]^2 + delta));
  }
}

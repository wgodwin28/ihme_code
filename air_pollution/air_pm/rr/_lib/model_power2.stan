...
model {
  ...
  for (n in 1:N) {
  pred_log_rr[n] <- log(
                        (1 + alpha * (1 - exp(-1 * beta * pow((exposure[n] - tmrel)/1e10, gamma)))) / 
                        (1 + alpha * (1 - exp(-1 * beta * pow((cf_exposure[n] - tmrel)/1e10, gamma))))
                    );
  }
...
generated quantities {
  ...
  predicted_RR[t] <- if_else(test_exposure[t] > tmrel, 
                      1 + alpha * (1 - exp(-1 * beta * pow((test_exposure[t] - tmrel)/1e10, gamma))), 
                      1.0);
  }
}
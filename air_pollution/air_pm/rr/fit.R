...
# source("/homes/jfrostad/_code/risks/air_pm/rr/fit.R", echo=T)
#***********************************************************************************************************************
...
if (Sys.info()["sysname"] == "Linux") {
  ...
  mcmc.cores <- 1 #number of MCMC chains to run in parallel (looks to run slower this waym so i set to 1 core only)
  arg <- commandArgs()[-(1:3)]  # First args are for unix use only
  arg <- c("power2_simsd_source", 7, 40, 1000) #toggle for targeted run
  ...
  arg <- c("power2_simsd_source", 7, 1, 1000)
  
}

# load packages, install if missing
pacman::p_load(data.table, ggplot2, grid, magrittr, parallel, reshape2, rstan, stringr)
...
#----IN/OUT-------------------------------------------------------------------------------------------------------------
##in##
data.dir <- file.path(home.dir, 'data/rr/prepped/', version)
...
##out##
output.dir <- file.path(home.dir, 'data/rr/output/', paste0(version, model))
  dir.create(output.dir, recursive=T)
graphs.dir <- file.path(home.dir, 'diagnostics/rr', paste0(version, model))
...
#note, using only the lite age groups for now, run full when satisfied with model fit
#age.cause <- age.cause[8:9,] #toggle to test only on copd/LRI

...
## Installing "RColorBrewer"if it isn't already available
install.packages("RColorBrewer")
library(RColorBrewer)

##require reshape package to rename variables
require(reshape)
...
setwd("J:/WORK/01_covariates/02_inputs/water_sanitation")
getwd()

## set directories
gpr_output_folder <- "J:/WORK/01_covariates/02_inputs/water_sanitation/data/02_Analyses/gpr/gpr_output/"
spacetime_output_folder <- "J:/WORK/01_covariates/02_inputs/water_sanitation/data/02_Analyses/smoothing/spacetime results/"
graph_folder <- "J:/WORK/01_covariates/02_inputs/water_sanitation/graphs/gpr/"

## specify and loop through models to plot
models <- c("s_covar","sewer_covar","w_covar","piped_covar",)
for (model in models) {  
    ...
    data <- read.dta(paste(gpr_output_folder, "gpr_results_", model, 
                           "_with_orig_data.dta", sep=""), convert.factor=F)
	data <- rename(data, c(location_name="countryname"))
    data <- rename(data, c(gbd_analytical_region_name="gbd_region_name"))
	data <- rename(data, c(gbd_analytical_superregion_id="gbd_superregion_name"))
    data <- data[, c("iso3", "countryname", "gbd_region_name", "gbd_superregion_name", "year", 
                     "actual_prev", "step1_prev", "step2_prev", "gpr_lower", 
                     "gpr_mean", "gpr_upper", "national", "plot")]
  
    ## outlier data
    if (grepl("w", model)) indicator <- "Water" 
	if (grepl("s", model))  <- "Sanitation"
	if (grepl("piped", model)) indicator <- "Piped" 
	if (grepl("sewer", model)) indicator <- "Sewer" 
    ...
    outliers <- outliers[, c("iso3", "countryname", "gbd_region_name", 
                             "gbd_superregion_name", "year", "actual_prev", "national", 
                             "plot")]
    names(outliers)[names(outliers)=="region"] <- "gbd_region_name"
    names(outliers)[names(outliers)=="super_region"] <- "gbd_superregion_name"
...
#    data$year <- (data$year-1980)/(2015-1980)
  ...
  ## set colors and styles for plot types
    ## types
    plot_types <- c("MICS", "DHS", "RHS", "LSMS", "Census", "IPUMS", "Other Survey", 
                    "Report", "JMP", "Subnational", "Outlier")
    ...
    ## styles
    plot_styles <- c(rep(16,6), 19, 17, 2, 3, 4)
...
#   c <- 1
      ...
      ## set graph structure
      par(mar=c(2,4,4,2))
      plot(x=seq(from=1980, to=2013, length.out=10),

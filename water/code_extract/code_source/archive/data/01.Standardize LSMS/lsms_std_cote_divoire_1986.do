// File Name: lsms_std_cote d'ivoire_1986.do

// File Purpose: Create standardized dataset with desired variables from LSMS Cote d'Ivoire 1986 survey
...
local dat_folder_country "CIV/1986"



** household weights dataset
	use "`dat_folder_lsms'/`dat_folder_country'/CIV_LSMS_1986_WEIGHTS.DTA", clear
...
** household services
	// water
	use "`dat_folder_lsms'/`dat_folder_country'/CIV_LSMS_1986_HOUSING_B1.DTA", clear
	tempfile hhwater
	save `hhwater', replace
		** hid
		
	// sanitation
	use "`dat_folder_lsms'/`dat_folder_country'/CIV_LSMS_1986_HOUSING_B2.DTA", clear
...
** merge
use `hhweight', clear
merge 1:1 hid using `hhwater'
drop _merge
merge 1:1 hid using `hhsanitation'
drop _merge
...
save "cote divoire_1986", replace

...
// File Purpose: Calculate prevalence of filter use and toilets with flush or sewer connection for MICS surveys
...
**drop surveys with no information on HWT 
drop if noinfo==1

**For now  - come back and fix MNE
drop if _n == 48 | _n==53 /*MNE 2005*/
...
local varlist iunimproved ipiped isrcedrnk itreat itreat2 itreat_improved itreat2_improved tr_improved untr_improved itreat_piped itreat2_piped tr_piped untr_piped ///
	itreat_unimproved itreat2_unimproved tr_unimproved untr_unimproved isanitation iflush 
...
forvalues filenum = 1(1)`maxobs' {
	...
	levelsof w_srcedr_o if (w_srcedr_s == "household connection" | w_srcedr_s == "public watersupply"), local(piped_water) 
		...
		** check whether variable has entries
		...
		if r(N) == 0 {	// no entries
			...
			if ! _rc {	// variable has label values
				...
				gen w_solar_i = 1 if `w_solar' == "E"
				replace w_solar_i = 0 if w_solar_i!=1
				}
				gen w_treat2_i = 0
				replace w_treat2_i = 1 if (w_bleach_i == 1 | w_treat2_i == 1) & w_treat_i!=1
				...
				replace w_piped_tr_i = 0 if (w_piped_treat_i != . & w_piped_treat2_i != . & w_piped_treat_i != 1)
				
				/*piped water + no HWT*/
				gen w_piped_untr_i = . 
				replace w_piped_untr_i = 1 if (w_piped_treat_i == 0 & w_piped_treat2_i == 0)
				replace w_piped_untr_i = 0 if (w_piped_treat_i != . & w_piped_treat2_i != . & w_piped_untr_i != 1)
				...
				/*other improved + any HWT*/
				gen w_improved_tr_i  = . 
				replace w_improved_tr_i = 1 if (w_improved_treat_i == 1 | w_improved_treat2_i == 1) 
				replace w_improved_tr_i = 0 if (w_improved_treat_i != . & w_improved_treat2_i != . & w_improved_treat_i != 1)
				
				/*other improved + no HWT*/
				gen w_improved_untr_i = . 
				replace w_improved_untr_i = 1 if (w_improved_treat_i == 0 & w_improved_treat2_i == 0)
				replace w_improved_untr_i = 0 if (w_improved_treat_i != . & w_improved_treat2_i != . & w_improved_untr_i != 1)
				...
				replace w_unimproved_tr_i = 0 if (w_unimproved_treat_i != .  &  w_unimproved_treat2_i != . & w_unimproved_tr_i!=1)
				
				/*unimproved source + no HWT*/
				gen w_unimproved_untr_i = . 
				replace w_unimproved_untr_i = 1 if (w_unimproved_treat_i == 0 & w_unimproved_treat2_i == 0)
				replace w_unimproved_untr_i = 0 if (w_unimproved_treat_i != . & w_unimproved_treat_i != . & w_unimproved_untr_i!=1)
				...
				//estimate survey-weighted prevalence of households that use solar/chlorine for disinfection by source; save outputs of interest in matrices
				if "`nopsu'" == "1" & "`noweight'" == "1" {
				...
				//estimate survey-weighted prevalence of households that use any HWT (boil/filter/solar/chlorine) by source; save outputs of interest in matrices
				if "`nopsu'" == "1" & "`noweight'" == "1" {
					mean w_`cat'_tr_i
				}
				else {
				...
				//estimate survey-weighted prevalence of households that DO NOT use any HWT (boil/filter/solar/chlorine) by source; save outputs of interest in matrices
				if "`nopsu'" == "1" & "`noweight'" == "1" {
					mean w_`cat'_untr_i
				}
				else {
		...
		** check whether variable has entries
		...
		if r(N) == 0 {	// no entries
			...
			if ! _rc {
				...
				gen flush_i = 0
				replace flush_i = 1 if (regexm(t_type_lab, "flush") | regexm(t_type_lab, "septic") | regexm(t_type_lab, "sewer"))
				summarize flush_i

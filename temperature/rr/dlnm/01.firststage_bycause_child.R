...
if(debug){
  ...
  cause <- "cvd"
  diag.dir <- "/home/j/temp/wgodwin/temperature/gasparrini_dlnm/diagnostics/mex/1"
  location <- "mex"
  country <- "Mexico"
}
...
formula <- death ~ cb + sdi + lights + offset(log(population)) + ns(date,df=dfseas*as.numeric(length(unique(year))))
print(formula)

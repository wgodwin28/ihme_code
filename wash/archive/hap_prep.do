...
local hap_dir "J:/temp/wgodwin/wash_exposure/03_upload/01_hap"
local hap_path "ubcov_tabulation_wgodwin.csv"
...
di in red "`hap_dir'"
	...
	drop if mean_adj<mean_unadj // drop the data points identified since they are not in the expected direction
...
*********************************************************************************
use "`j'/WORK/05_risk/risks/air_hap/01_exposure/01_data audit/02_data/Complete Maln Database ALL datasources/database_2015_complete_25Jul2016.dta", clear
...
duplicates drop location_name year_start nid, force // kenya has a dup for unknown reason 
drop if mean == 1 | mean == 0
...
replace data = . if nid==81044 & ihme_loc_id=="IND" // In the report, only urban, rural split for country level, replace data = . for now if have time do urban/rural crosswalk
replace data = . if nid==124072 // China subnational, I found the data from this survey generally lower than others, replace data = . for now , ask Erica to get access to the data 
replace data = . if nid==798 & ihme_loc_id=="ARM" // I did not see the 2002 data under the series folder and there is not much information on GHDx
replace data = . if nid==794 & ihme_loc_id=="ARM" // I did not see the 2002 data under the series folder and there is not much information on GHDx
...
replace data = . if nid==142027 & ihme_loc_id=="ARM" // Data actually from Households Integrated Living Conditions Survey which I trust less and still need to investigate it 
replace data = . if nid==22786 & ihme_loc_id=="ARM" // do not have fuel variable

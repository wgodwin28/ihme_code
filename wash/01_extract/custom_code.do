...
//Read in ratios to split ambiguous strings calculated by Ani
preserve
	//Sanitation ratios
	 import delimited "`j'/WORK/11_geospatial/wash/definitions/cw_sani.csv", clear
	 tempfile sani_cw
	 save `sani_cw', replace
	 keep if iso3 == "$ihme_loc_id"
	 local region = reg
	 if sources < 5 {
	 	use `sani_cw', clear
	 	keep if reg == "`region'"
	...
	//Water ratios
	 import delimited "`j'/WORK/11_geospatial/wash/definitions/cw_water.csv", clear
	 tempfile water_cw
...
// Improved and piped water indicator generation
	...
	//	if !_rc {
	//		gen handwashing = .
	//		replace handwashing = 1 if wash_water_imp == 1 & hw_station == 1
...
if !_rc {
	...
	**replace wash_filter_treat_prop = 0 if wash_filter_treat_prop != 1 & hwt == 1
	...
	**gen wash_solar_treat_prop = 1 if w_solar == 1 | w_bleach == 1
	**replace wash_solar_treat_prop = 0 if wash_solar_treat_prop != 1 & wash_filter_treat_prop == 0
	gen wash_filter_treat_prop = 1 if w_boil == 1 | w_filter == 1
...
if !_rc {
	...
	if "`survey'" == "UNICEF_MICS"{
		...
		**replace wash_filter_treat_prop = 0 if wash_filter_treat_prop != 1 & hwt == 1
		...
		//boil/Filter category
		**gen wash_solar_treat_prop = 1 if w_bleach == 1
		**replace wash_solar_treat_prop = 0 if wash_solar_treat_prop != 1 & wash_filter_treat_prop == 0
		gen wash_filter_treat_prop = 1 if w_boil == 1 | w_filter == 1
		replace wash_filter_treat_prop = 0 if wash_filter_treat_prop != 1 & wash_no_treat == 0
	}

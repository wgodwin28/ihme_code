...
// Set relevant locals
local version 2
local in_dir "J:/WORK/05_risk/risks/wash_water/data/exp/01_data_audit/piped_water_quality"
local out_dir "J:/WORK/05_risk/risks/wash_water/data/exp/03_model/`version'/"
adopath + "`j'/temp/central_comp/libraries/current/stata"

// Import dataset from Bain et al. 2014
	import delimited "`in_dir'/bain_db.csv", clear
	drop v*
...
// Import dataset with other additional water quality data points
	import delimited "`in_dir'/eu_data.csv", clear // downloaded from EU water quality website
	tempfile additional
...
// Clean and prep for ST-GPR
	...
	drop if location_id == .
	export delimited "`out_dir'/prop_fecal.csv", replace 
...
// Prep piped water as a covariate for prop_fecal model
	...
	local base_dir "J:/temp/wgodwin/gpr_output/run1"
	local out_dir "J:/WORK/05_risk/risks/wash_water/data/exp/03_model/best_2016"
	import delimited "`base_dir'/wash_water_imp_3771.csv", clear
	...
	import delimited "`base_dir'/wash_water_piped_3772.csv", clear
	replace gpr_mean = .99 if gpr_mean >= 1
	...
	preserve
		import delimited "`base_dir'/prop_fecal_base.csv", clear
		tempfile fecal
		save `fecal', replace
	restore

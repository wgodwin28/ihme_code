// Purpose: Conduct meta-regression to derive RR's for breastfeeding using Horta et al., 2014
// Date: 1/23/17
...
// Set locals
	local in_dir "J:/WORK/05_risk/risks/nutrition_breast/rr/horta/data"
	adopath + "`j'/temp/central_comp/libraries/current/stata"
	local out_dir "/share/epi/risk/temp/breastfeeding/rr"
...
***************************************************************
set seed 12345
import delimited "`in_dir'/breast_meta.csv", clear
...
// subset dataset appropriately
	...
	//keep if outcomemeasure == "Mortality"
	keep if age_lower_years <= .5

// Experiment with subsetting the dataset on outcome and run separate meta-analysis for LRI and diarrhea
	...
	// Loop through each outcome and generate necessary specifications for save_results
		foreach outcome in diar lri {
			...
			keep if regexm(cat, "`outcome'")
				...
				//replace morbidity = 1 if regexm(cat, "morb")
				// replace morbidity = 0 if morbidity == .
				// replace mortality = 1 if regexm(cat, "mort")
...
// Work around to duplicate out for each age_group_id
	...
	** Save mean/lower/upper on the J drive**
	...
	foreach sex in 1 2 {
		replace sex_id = `sex'
		export delimited "`out_dir'/non_exclusive/rr_1_1990_`sex'.csv", replace
	}
	...
	*************************************
	if `save_results' == 1 {
		local dat_dir "/share/epi/risk/temp/breastfeeding/rr/non_exclusive"
		quietly run "/home/j/temp/central_comp/libraries/current/save_results.do"
		save_results, modelable_entity_id(9052) description(update with separate regressions for lri and diarrhea) in_dir(`dat_dir') risk_type("rr")
	}
...
// Transform and conduct regression
	...
	// xi: metareg log_rr i.outcomemeasure, wsse(se_log) eform
...
// Work around to duplicate out for each age_group_id
	...
	** Save mean/lower/upper on the J drive**
	...
	foreach sex in 1 2 {
		replace sex_id = `sex'
		export delimited "`out_dir'/discontinued/rr_1_1990_`sex'.csv", replace
	}
	...
	*************************************
	if `save_results' == 1 {
		local dat_dir "/share/epi/risk/temp/breastfeeding/rr/discontinued"
		quietly run "/home/j/temp/central_comp/libraries/current/save_results.do"
		save_results, modelable_entity_id(9053) description(cleaned some studies from meta-regression) in_dir(`dat_dir') risk_type("rr")
	}

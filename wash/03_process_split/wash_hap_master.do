...
// Additional Comments: 
// do /snfs2/HOME/wgodwin/risk_factors2/wash/03_process_split/wash_hap_master.do
...
// Set relevant locals
	...
	local code_folder 		"/snfs2/HOME/wgodwin/risk_factors2/wash/03_process_split"
...
// Prep location specific files so lower level script doesn't have to load in whole dataset each time
	foreach loc of local locations {
		foreach exp of local exposures {
			! qsub -N `exp'_`loc'_split -P proj_custom_models -pe multi_slot 1 `logs' "`stata_shell'" "`code_folder'/location_split_save.do" "`location_dir' `input_dir' `loc' `exp'"
		}
	}	

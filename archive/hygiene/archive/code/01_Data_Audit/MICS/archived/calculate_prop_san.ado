...
mata: `survey'_vars=st_sdata(.,("countryname", "iso3", "startyear", "endyear", "filedir", "filename", ///
	"psu", "weight", "w_hwss", "w_barsoap", "w_liqsoap", "w_detergent", "w_water", "t_type"))
local maxobs = _N
...
**local filenum 1
	...
	mata: st_local("w_barsoap", `survey'_vars[`filenum', 10])
	mata: st_local("w_liqsoap", `survey'_vars[`filenum', 11])
	mata: st_local("w_detergent", `survey'_vars[`filenum', 12])
	mata: st_local("w_water", `survey'_vars[`filenum', 13])
	...
	use "`san_dataloc'/`survey'/varlabels_mics4_`prevtype'", clear
	...
	use `psu' `weight' `w_hwss' `w_barsoap' `w_liqsoap' `w_detergent' `w_water' `t_type' using "`filedir'/`filename'", clear
		...
		if r(N) == 0 | _rc != 0 {	// no entries
			...
			if ! _rc {
				...
				**observed handwashing station
				...
				replace w_station_i = 0 if (`w_hwss'!= 1 & `w_hwss'!=9)
				replace w_station_i = . if (`w_hwss' == 9 | `w_hwss' == .)
				
				**observed water at handwashing station
				gen w_water_i = 1 if `w_water'== 1
				replace w_water_i = . if (`w_water' == . | `w_water' == 9)
				replace w_water_i = 0 if `w_water' == 2 
				
				**observed soap at handwashing station 
				gen w_soap_i = 1 if (`w_barsoap' == "A"	| `w_liqsoap'== "B" | `w_detergent'== "C")
				replace w_soap_i = . if (`w_barsoap' == "?"  & `w_liqsoap'=="?" & `w_detergent'=="?")
				replace w_soap_i = 0 if (`w_barsoap' == ""	& `w_liqsoap'== "" & `w_detergent'== "")
				
				**observed handwashing station + water + soap
				gen w_hwss_i = 1 if (w_station_i == 1 & w_water_i == 1 & w_soap_i == 1)
				replace w_hwss_i = 0 if w_hwss_i!=1
				replace w_hwss_i = . if (w_station_i == . | w_water_i == . | w_soap_i == .)
				
				//create variable combining handwashing and toilet type
				gen hwws_unimproved_i = 1 if (w_hwss_i == 1 & t_type_i == 0 & t_sewer_i == 0)
				replace hwws_unimproved_i = 0 if (w_hwss_i == 0 & t_type_i == 0 & t_sewer_i == 0)
				
				gen hwws_improved_i = 1 if (w_hwss_i == 1 & t_type_i == 1 & t_sewer_i == 0)
				replace hwws_improved_i = 0 if (w_hwss_i == 0 & t_type_i == 1 & t_sewer_i == 0)
				
				gen hwws_sewer_i = 1 if (w_hwss_i == 1 & t_type_i == 0 & t_sewer_i == 1)
				replace hwws_sewer_i = 0 if (w_hwss_i == 0 & t_type_i == 0 & t_sewer_i == 1)
...
use "`dataloc'/`survey'/varlist_`survey'", clear
drop psu weight urban w_* t_* noinfo

...
**Author: Astha KC
**Date: July 9 2015
**Edited: Will Godwin- to reflect meta-analysis update
**Date: April 7 2017
...
// Set relevant locals
	local out_dir_draws		"/share/epi/risk/temp"
...
// Prep regional codes 
	...
	keep location_id ihme_loc_id super_region_name region_name location_name level parent_id region_id
	tempfile loc
	save `loc', replace
...
******************************************************
 // EDIT: New relative risks are calculated in house by adding a few new studies to Wolf et al, 2014 meta-analysis -WG
...
*************************************************************************************************
	...
	**piped - hwt** (quality varies by region)
	gen rr_cat7 = (`rr_piped')/(rr_piped_hi_*rr_filter_)
	
	**piped  + chlorine/solar** (quality varies by region)
	gen rr_cat8 = (`rr_piped'*rr_chlorine_)/(rr_piped_hi_*rr_filter_)
	
	**piped + boil/filter (quality varies by region)

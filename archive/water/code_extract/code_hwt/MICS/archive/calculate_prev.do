		...
		if r(N) == 0 {	// no entries
			display "variable empty"
				...
				**encode `w_boil', gen(w_boil_i)
				...
				//estimate survey-weighted prevalence of chlorination/bleaching; save outputs of interest in matrices
					if "`nopsu'" == "1" & "`noweight'" == "1" {
					mean w_bleach_i
				}
				else {
				...
				//estimate survey-weighted prevalence of solar disinfection; save outputs of interest in matrices
					if "`nopsu'" == "1" & "`noweight'" == "1" {
					mean w_solar_i
				}
				else {
				...
				//estimate survey-weighted prevalence of solar disinfection/chlorination; save outputs of interest in matrices
					if "`nopsu'" == "1" & "`noweight'" == "1" {
					mean w_treat2_i
				}
				else {
...
** open file with list of surveys - add newly calculated prevalence data from the matrices
use "`dataloc'/`survey'/datfiles_`survey'", clear
getmata ifilter_mean ifilter_sem iboil_mean iboil_sem itreat_mean itreat_sem ibleach_mean ibleach_sem ///
	isolar_mean isolar_sem itreat2_mean itreat2_sem iflush_mean iflush_sem nopsu noweight, id(filename)

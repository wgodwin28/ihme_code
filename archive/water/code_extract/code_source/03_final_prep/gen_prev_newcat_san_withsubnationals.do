...
local san_CHN			"$j/WORK/01_covariates/02_inputs/water_sanitation/data/subnational/CHN"
local san_MEX			"$j/WORK/01_covariates/02_inputs/water_sanitation/data/subnational/MEX"
...
//Prep dataset
**Sanitation**
use "`source_results'/gpr_results_s_covar_with_orig_data.dta", clear
...
use "`san_CHN'/sanitation_chn.dta", clear
keep iso3 year pred*
forvalues n = 1/1000 {
	rename pred`n' isanitation_`n'
	gen iunimp_`n' = 1 - isanitation_`n'
		}
...
use "`san_MEX'/sanitation_MEX_draws.dta", clear
keep iso3 year *pred*
forvalues n = 1/1000 {
	rename sanitation_prov_pred`n' isanitation_`n'
	gen iunimp_`n' = 1 - isanitation_`n'
		}
...
***Sewer**
use "`source_results'/gpr_results_sewer_covar_with_orig_data.dta", clear
keep iso3 year gpr_draw*
duplicates drop
forvalues n = 1/1000 {
...
**china**
use "`san_CHN'/sewer_chn.dta", clear
keep iso3 year pred*
forvalues n = 1/1000 {
	rename pred`n' isewer_`n'
		}
...
**mexico**
use "`san_MEX'/sewer_MEX_draws.dta", clear
keep iso3 year *pred*
forvalues n = 1/1000 {
	rename sewer_prov_pred`n' isewer_`n'
		}
...
**Create a template for subnational GBR
	...
	append using `san_cats'
	save `san_cats', replace 

/* merge country iso3s /region variables*/
merge m:1 iso3 using `codes', keepusing(location_id gbd_country_iso3 gbd_analytical_region_name gbd_analytical_superregion_id) keep(1 3) nogen

**format subnational location variables
tostring(location_id), replace
gen new_iso3 = gbd_country_iso3 + "_" + location_id if gbd_country_iso3!=""
replace iso3 = new_iso3 if gbd_country_iso3!=""
drop new_iso3 
...
**save data**
save "`final_output'/newcat_final_prev_san_`date'.dta", replace

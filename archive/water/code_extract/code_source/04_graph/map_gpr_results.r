...
data_full <- read.dta("J:/WORK/01_covariates/02_inputs/water_sanitation/graphs/maps/gpr_results_for_mapping.dta", convert.factor=F)
...
indicators <- c("Water", "Piped")
indicators <- c("Sanitation", "Sewer")  
...
for (j in 1:length(types)) {
  ...
  for (i in 1:length(indicators)) {
    ...
    indicator <- indicators[i]
  	...
  	##if (grepl("W", indicator)) graph <- "(A)" else graph <- "(B)"
	if (grepl("W", indicator)) graph <- "Improved source" else graph <- "Piped source"
	##if (grepl("San", indicator)) graph <- "Improved sanitation" else graph <- "Sewer connection"
    ...
    if (type == types[j]) {
      limit_vec = c(0, 20, 40, 60, 80, 100)
      label_vec = c("<20%", "20%-40%", "40%-60%", "60%-80%", ">80%")

...
syntax varlist [if] [in], [gen(namelist) replace cfindic(namelist) back carryalong(varlist) strict ///
	dynamic_condition(string) debug]
...
foreach v of local varlist {
	...
	if "`debug'" ~= "" {
		disp "~~~~"
		disp "`by' replace `ggg' = `ggg'[_n-1] if _n>1 & `touse' & mi(`ggg') & ~mi(`ggg'[_n-1]) `strictcondition' `dynamic_condition2'"
	}

	`by' replace `ggg' = `ggg'[_n-1] if _n>1 & `touse' & mi(`ggg') & ~mi(`ggg'[_n-1]) ///

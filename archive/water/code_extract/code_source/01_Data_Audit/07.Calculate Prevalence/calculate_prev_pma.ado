...
mata: `survey'_vars=st_sdata(.,("location_name", "ihme_loc_id", "year_id", "filedir", "filename", ///
...
// local filenum 7
	...
	mata: st_local("location_name", `survey'_vars[`filenum', 1])
	mata: st_local("ihme_loc_id", `survey'_vars[`filenum', 2])
	mata: st_local("year_id", `survey'_vars[`filenum', 3])
	mata: st_local("filedir", `survey'_vars[`filenum', 4])
	mata: st_local("filename", `survey'_vars[`filenum', 5])
	mata: st_local("psu", `survey'_vars[`filenum', 6])
	mata: st_local("weight", `survey'_vars[`filenum', 7])
	mata: st_local("w_srcedrnk", `survey'_vars[`filenum', 8])
	mata: st_local("t_type", `survey'_vars[`filenum', 9])

	di _newline _newline "**********************************************************************************"
	di "countryname: `location_name'" _newline "filename: `filename'" _newline "filenum: `filenum'"
	di "**********************************************************************************"
...
/*		decode `w_srcedrnk', gen(w_srcedrnk)
		decode `t_type', gen(t_type)
	foreach var in w_srcedrnk t_type {
	// if "`ihme_loc_id'"" == "GHA" {
		replace w_sd_lab = substr(w_sd_lab, 4, .) if "`ihme_loc_id'" == "GHA"
		replace `var' = substr(`var', 4, .) if "`ihme_loc_id'" == "KEN" & "`filename'" != "KEN_PMA2020_2014_R1_HHQFQ_Y2016M05D13.DTA"
		replace w_srcedrnk = substr(w_srcedrnk, 4, .)
		replace w_sd_lab = substr(w_sd_lab, 4, .) if ihme_loc_id == "UGA" & year_id == "2015"
		replace w_sd_lab = substr(w_sd_lab, 4, .) if ihme_loc_id == "ETH" & year_id == "2015" 
		replace w_sd_lab = subinstr(w_sd_lab, "_", " ", .)
		replace w_sd_lab = subinstr(w_sd_lab, "-", " ", .)
		...
		** check whether variable has entries
		...
		if r(N) == 0 {	// no entries
			...
			if ! _rc {	// variable has label values
				...
				replace w_sd_lab = strlower(w_sd_lab)
				replace w_sd_lab = substr(w_sd_lab, 4, .) if "`ihme_loc_id'" == "GHA"
				replace w_sd_lab = substr(w_sd_lab, 4, .) if "`ihme_loc_id'" == "KEN" & "`filename'" != "KEN_PMA2020_2014_R1_HHQFQ_Y2016M05D13.DTA"
				replace w_sd_lab = substr(w_sd_lab, 4, .) if "`ihme_loc_id'" == "UGA" & "`year_id'" == "2015"
				replace w_sd_lab = substr(w_sd_lab, 4, .) if "`ihme_loc_id'" == "ETH" & "`year_id'" == "2015" 
				replace w_sd_lab = subinstr(w_sd_lab, "_", " ", .)
				replace w_sd_lab = subinstr(w_sd_lab, "-", " ", .)
				...
				foreach type of local unimproved_water {
					replace w_srcedrnk_i = 0 if w_sd_lab == "`type'"  | w_sd_lab == "sachet"
				}
		...
		** check whether variable has entries
		...
		if r(N) == 0 {	// no entries
			...
			if ! _rc {
				...
				replace t_type_lab = strlower(t_type_lab)
				replace t_type_lab = substr(t_type_lab, 4, .) if "`ihme_loc_id'" == "GHA"
				replace t_type_lab = substr(t_type_lab, 4, .) if "`ihme_loc_id'" == "KEN" & "`filename'" != "KEN_PMA2020_2014_R1_HHQFQ_Y2016M05D13.DTA"
				replace t_type_lab = substr(t_type_lab, 4, .) if "`ihme_loc_id'" == "UGA" & "`year_id'" == "2015"
				replace t_type_lab = substr(t_type_lab, 4, .) if "`ihme_loc_id'" == "ETH" & "`year_id'" == "2015" 
				replace t_type_lab = subinstr(t_type_lab, "_", " ", .)
				replace t_type_lab = subinstr(t_type_lab, "-", " ", .)
				...
				foreach type of local unimproved_sanitation {
					...
					replace t_type_i = 0 if t_type_i == . & t_type_lab != ""
				}
				...
				//disaggregation of aggregated survey responses for water
				global filename = "`filename'"
				do "J:/WORK/01_covariates/02_inputs/water_sanitation/code/01_Data_Audit/07.Calculate Prevalence/disaggregate_categories_san.do"
...
rename year_id endyear
gen startyear = endyear

// File Name: lookfor_vars_mics4.do

// File Purpose: Look for appropriate variables in MICS 4 surveys
...
keep if svyver == "4"
save "`dat_folder_new_mics'/datfiles_mics4", replace
...
save "`dat_folder_new_mics'/varlist_temp_mics4", replace	
...
** clean up entries with more than one variable listed
	...
	replace urban = "HH6" if regexm(urban, "HH6")
	replace urban = "HH6" if regexm(urban, "area") /*fix MNG 2010*/
	replace urban = "HH6" if urban==""
...
** save 
compress
save "`dat_folder_new_mics'/varlist_mics4", replace
